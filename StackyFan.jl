...
function coneRayDecomposition(cone,rayMatrix,ray,stack)
    ...
    # Express the input ray in terms of the stackified cone generators
    B=Polymake.common.null_space(hcat(transpose(coneRays),-ray))
    N=convert(Array{Int64,1},vec(B))
    if size(N,1)==0
        error("The given ray is not in the span of the cone generators.")
    end
    # Since the nullspace has arbitrary sign, fix it so the coefficients are all positive
    if N[end]<0 
        N*=-1
    end
    pop!(N)
    ...
    for i in 1:size(N,1)
        # Rewrite the coefficients vector in terms of all the rays in rayMatrix, by padding with zeros when appropriate.
        out[cone[i]]=N[i] 
    end
    return out
end
